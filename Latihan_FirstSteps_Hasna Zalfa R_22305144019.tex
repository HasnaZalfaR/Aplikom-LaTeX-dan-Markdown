...
\end{eulercomment}
\eulerimg{24}{images/Latihan_FirstSteps_Hasna Zalfa R_22305144019-004.png}
\eulersubheading{}
\begin{eulercomment}
...
\eulersubheading{}
\begin{eulerprompt}
>x=7; x^5+3*x^4-x^3+7*x^2-x+13
\end{eulerprompt}
\begin{euleroutput}
  24016
\end{euleroutput}
\begin{eulerprompt}
>x=1; y=4; z=3; x^1+y^2+z^2+x*y*z+3*x*y+2*y*z+5*x*z-2*x-y+z-1
\end{eulerprompt}
\begin{euleroutput}
  85
\end{euleroutput}
...
\end{eulercomment}
\begin{eulerttcomment}
 a. f(x) = x^2+(1/x^3), dengan batas atas = -2 dan batas bawah = -4
 b. g(x) = 2*sin(x)+3*sin(2*x)-cos(3*x)
 c. h(x) = (x^4-8)/x^2, dengan batas atas = 4 dan batas bawah = 1
\end{eulerttcomment}
\begin{eulerprompt}
>function f(x) &= x^2+(1/x^3)
\end{eulerprompt}
\begin{euleroutput}
...
\end{euleroutput}
\begin{eulerprompt}
>function g(x) &= 2*sin(x)+3*sin(2*x)-cos(3*x)
\end{eulerprompt}
\begin{euleroutput}
  
                    - cos(3 x) + 3 sin(2 x) + 2 sin(x)
  
\end{euleroutput}
\begin{eulerprompt}
>function G(x) &= integrate(g(x),x)
\end{eulerprompt}
\begin{euleroutput}
